        ...
        if (profileData) {
          ...
          if (profileData.baseline_completed) {
            ...
            if (latestComplete?.completed_at) {
              ...
              const canRedo = daysSince < 90 && redoCount < 1
              const redoUsed = redoCount >= 1

              let buttonText = 'Start New Baseline'
              let statusMessage = 'Next quarterly baseline available.'

              if (daysSince < 90) {
                if (canRedo) {
                  buttonText = 'Redo Baseline'
                  statusMessage = 'You have ONE redo opportunity within 90 days to update scores.'
                } else if (redoUsed) {
                  buttonText = `Available in ${Math.ceil(90 - daysSince)} days`
                  statusMessage = 'Redo already used. Next quarterly baseline available in ' + Math.ceil(90 - daysSince) + ' days.'
                }
              }

              setBaselineInfo({
                completed: true,
                daysSinceCompletion: Math.round(daysSince),
                canRedo,
                redoUsed,
                buttonText,
                statusMessage
              ...
              <div className="mb-6">
                ...
                {baselineInfo.completed && (
                  <p className="text-sm text-black/50 mt-2">
                    Last completed {baselineInfo.daysSinceCompletion} days ago. {baselineInfo.statusMessage}
                  </p>
                )}
              </div>
              {profile?.baseline_completed ? (
                baselineInfo.redoUsed && baselineInfo.daysSinceCompletion < 90 ? (
                  <button
                    disabled
                    className="inline-block px-6 py-3 border-2 border-gray-300 text-gray-400 rounded-lg font-semibold cursor-not-allowed"
                  >
                    {baselineInfo.buttonText}
                  </button>
                ) : (
                  <a
